...
function controls:add_param(config)
  ...
  params:set_action(config.name, engine[config.name])
end
...
local function new_poly_sub()
  ...
  for i = 1, params["count"] do
    ...
    c.sliders[i] = math.floor(scale(params:get(i), c.scalers[name].min, c.scalers[name].max, 0, slider_max))
  end

  -- calculate meter offset
  c.meter_x = (126 - #c.sliders * 4) / 2
...
function controls:enc(n, delta)
  ...
  elseif n == 3 then
    ...
    self.sliders[self.edit] = math.floor(scale(params:get(self.edit), self.scalers[name].min, self.scalers[name].max, 0, slider_max))
  end
...
function controls:redraw()
  ...
  if (self.ticks > 0) then
    ...
    screen.move(64, 60)
    screen.text_center(params:get_name(self.index))
  end
